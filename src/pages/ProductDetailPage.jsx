...
export default function ProductDetailPage({ product, onNavigate, onSelectProduct, onQuickView, onOpenVideo, onShowToast }) {
  ...
  // Sync state whenever selected product prop changes!
  useEffect(() => {
    if (product) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-28 md:pb-20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 pt-2 sm:pt-4">
        ...
        <div className="lg:col-span-6 flex flex-col-reverse sm:flex-row gap-4">
          ...
          <div className="flex sm:flex-col gap-2.5 overflow-x-auto sm:overflow-y-auto shrink-0 py-1 max-w-full">
            {gallery.map((imgUrl, idx) => (
              <button
                ...
                className={`w-14 h-14 sm:w-18 sm:h-18 rounded-xl sm:rounded-2xl overflow-hidden border-2 transition-all shrink-0 bg-white ${
                  currentImage === imgUrl 
                    ? 'border-brand-gold ring-2 sm:ring-4 ring-brand-gold/20 scale-105 shadow-md' 
                    : 'border-brand-border-light hover:border-brand-gold/50 opacity-80 hover:opacity-100'
                }`}
              >
          ...
          <div 
            className="relative flex-1 aspect-square rounded-2xl sm:rounded-3xl overflow-hidden bg-white border border-brand-gold/30 shadow-xl group cursor-crosshair"
          ...
          >
            ...
            <button
              ...
              className={`absolute top-3 right-3 sm:top-4 sm:right-4 p-3 rounded-full backdrop-blur-md shadow-xl transition-all ${
                isLiked ? 'bg-rose-50 text-rose-600' : 'bg-white/80 text-gray-600 hover:text-rose-600 hover:bg-white'
              }`}
            >
        ...
        <div className="lg:col-span-6 space-y-5 sm:space-y-6">
          <div>
            <div className="flex items-center justify-between text-xs">
              <span className="text-amber-700 font-bold uppercase tracking-widest bg-amber-50 px-2.5 py-1 rounded-md border border-amber-200/60 flex items-center gap-1 text-[10px] sm:text-xs">
                <Sparkles className="w-3.5 h-3.5 text-amber-600" /> NECBDC Certified Craft
              </span>
              <div className="flex items-center gap-1.5 text-amber-500 font-bold text-xs sm:text-sm">
                <Star className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-amber-400 text-amber-400" />
                <span>{product.rating}</span>
                <span className="text-brand-gray font-normal text-[10px] sm:text-xs">({product.reviewsCount} reviews)</span>
              </div>
            </div>

            <h1 className="brand-font-serif text-2xl sm:text-4xl font-bold text-brand-charcoal mt-2 sm:mt-3">
              {product.title}
            </h1>
            ...
            <div className="mt-3 flex items-baseline gap-3">
              ...
              {product.originalPrice && (
                <span className="text-xs sm:text-base text-brand-gray line-through">
                  ₹{product.originalPrice.toLocaleString('en-IN')}
                </span>
              )}
              <span className="text-[10px] font-bold text-emerald-900 bg-emerald-100 px-2.5 py-0.5 rounded-md border border-emerald-200">
                Save ₹{((product.originalPrice || product.price + 300) - product.price).toLocaleString('en-IN')}
              </span>
            </div>
            <p className="text-[11px] sm:text-xs text-brand-gray font-medium mt-1">Inclusive of all taxes & free eco-packaging</p>
          </div>

          <p className="text-xs sm:text-sm text-brand-gray leading-relaxed border-t border-brand-border-light pt-4">
            {product.description}
          </p>

          {/* Value props bar in PDP */}
          <div className="grid grid-cols-3 gap-2 sm:gap-3 py-3 border-y border-brand-border-light/80 text-center text-[10px] sm:text-[11px] text-brand-charcoal font-bold bg-white rounded-2xl p-2 border border-brand-gold/20">
          ...
          <div className="hidden sm:block space-y-4 pt-2">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center border border-brand-gold/40 rounded-2xl bg-white shadow-xs">
                <button
                  onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="px-4 py-3.5 text-sm font-bold text-brand-charcoal hover:bg-brand-beige rounded-l-2xl"
                >
                  <Minus className="w-3.5 h-3.5" />
                </button>
                <span className="px-4 text-sm font-bold text-brand-charcoal min-w-[36px] text-center">
                  {quantity}
                </span>
                <button
                  onClick={() => setQuantity(quantity + 1)}
                  className="px-4 py-3.5 text-sm font-bold text-brand-charcoal hover:bg-brand-beige rounded-r-2xl"
                >
            ...
            <button
              ...
              className="w-full bg-white hover:bg-brand-beige text-brand-green border-2 border-brand-gold text-xs sm:text-sm font-bold py-3.5 px-6 rounded-2xl transition-all uppercase tracking-widest shadow-sm"
            >
              BUY NOW
            </button>
          </div>
          ...
          {/* Delivery & COD Badges */}
          <div className="bg-white p-4 rounded-2xl border border-brand-gold/30 space-y-2 text-xs text-brand-charcoal shadow-xs">
            <div className="flex items-center gap-2">
              <Truck className="w-4 h-4 text-emerald-800 shrink-0" />
              <span>Estimated Delivery: <strong className="text-brand-green">3 - 5 working days</strong></span>
            </div>
            <div className="flex items-center gap-2">
              <RefreshCw className="w-4 h-4 text-emerald-800 shrink-0" />
              <span>COD Available | Easy 7 Days Hassle-Free Returns</span>
            </div>
          </div>

          {/* Interactive Accordions */}
          <div className="border-t border-brand-border-light pt-2 space-y-2 text-xs">
            
            <div className="border-b border-brand-border-light pb-2">
              <button
                onClick={() => toggleAccordion('details')}
                className="w-full py-3 flex justify-between items-center font-bold text-brand-charcoal hover:text-brand-green"
              >
                <span>Product Details</span>
                <ChevronDown className={`w-4 h-4 transition-transform ${activeAccordion === 'details' ? 'rotate-180' : ''}`} />
              </button>
              {activeAccordion === 'details' && (
                <ul className="pb-3 space-y-1.5 text-brand-gray pl-4 list-disc animate-fadeIn">
                  {product.details ? (
            ...
            <div className="border-b border-brand-border-light pb-2">
              ...
              {activeAccordion === 'shipping' && (
                <p className="pb-3 text-brand-gray leading-relaxed animate-fadeIn">
      ...
      <div className="sm:hidden fixed bottom-12 inset-x-0 bg-white/95 backdrop-blur-xl border-t border-brand-gold/30 p-3 shadow-2xl z-30 flex items-center gap-2">
        <button
          onClick={handleAddToCart}
      ...
      {relatedProducts.length > 0 && (
        <div className="mt-16 sm:mt-20 space-y-6 border-t border-brand-border-light pt-12">
          <h2 className="brand-font-serif text-2xl font-bold text-brand-charcoal">
            You May Also Like
          </h2>
